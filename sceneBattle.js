...
let SceneBattle = new Phaser.Class({
    ...
    updateIcons: function (points, effects)
    {
        ...
        var table = Give_effects_table(cardData.basic_fire_1, cardData.basic_fire_1, effects);  //2 pierwsze argumenty to karty bez efektów
        for (var i = 0; i < 2; i++)
        {
            if (table[i] > 0)
            {
...
let Hand = new Phaser.Class({
    ...
    initialize:
        function Hand(scene, size, deck, pos_y, index)
        {
            ...
            this.drawUntilLimit();
            this.cancelButton = new Button(this.scene, "cancel", 1100, 800 - pos_y, 0.5, "button_cancel", this);
            this.replaceIcons = [new Icon(this.scene, 1100, 750 - pos_y, 0.5, 'sym_icon_replace_can'), new Icon(this.scene, 1100, 750 - pos_y, 0.5, 'sym_icon_replace_must')];
...
let Battle = new Phaser.Class({
    ...
    Add_card: function (new_card, cards_index)
    {
        this.cards[cards_index] = new_card; 
        if (this.cards[0] != null && this.cards[1] != null) 
        {
            ...
            {
                case 1:
                    console.log(this.cards[0].data.displayName + " wins!");
                    this.points[this.cards[0].data.element - 1]++;
                    break;
                case -1: console.log(this.cards[1].data.displayName + " wins!");
                    this.points[this.cards[1].data.element + 2]++;
                    break;
            ...
            var afterturn_table = Give_afterturn_effects_table(this.effects);   //efekty po turze (głównie modyfikujące rękę)
