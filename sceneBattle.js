...
let SceneBattle = new Phaser.Class({
    ...
    updateIcons: function (points, effects)
    {
        ...
        if (inturnTable[4] === -1)
            this.conditionIcons[0].visual.visible = true;
        else
            this.conditionIcons[0].visual.visible = false;
        if (inturnTable[5] === -1)
            this.conditionIcons[1].visual.visible = true;
        else
            this.conditionIcons[1].visual.visible = false;
        if (inturnTable[6] === 1)
            this.conditionIcons[2].visual.visible = true;
        else
            this.conditionIcons[2].visual.visible = false;
        if (inturnTable[7] === 1)
            this.conditionIcons[3].visual.visible = true;
        else
            this.conditionIcons[3].visual.visible = false;
...
let Card = new Phaser.Class({
    ...
    initialize:
        function Card(scene, data, x, y, scale, hand)
        {
            ...
            this.visual.setDepth(5 + scale * 100);  //aby powiększenia kart były zawsze na wierzchu
...
let Battle = new Phaser.Class({
    ...
    addCard: function (new_card, user)
